...
1. Find the smallest and largest values, the median, and the first and third quartile for the night class.
1. Tìm các giá trị nhỏ nhất và lớn nhất, trung vị, cùng tứ phân vị thứ nhất và thứ ba cho lớp ban đêm.
1. For each data set, what percentage of the data is between the smallest value and the first quartile? the first quartile and the median? the median and the third quartile? the third quartile and the largest value? What percentage of the data is between the first quartile and the largest value?
...
1. Create a box plot for each set of data. Use one number line for both box plots.
1. Tạo một biểu đồ hộp cho mỗi tập dữ liệu. Sử dụng một trục số cho cả hai biểu đồ hộp.
1. Which box plot has the widest spread for the middle 50% of the data (the data between the first and third quartiles)? What does this mean for that set of data in comparison to the other set of data?
